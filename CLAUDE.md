...
2. **Pom watcher** (`pomwatch.go`): fsnotify on the projects dir and each project root only (never `target/`, so build churn can't flood it). Hashes rebuild-relevant pom content — dependencies/parent plus, when resource filtering is on, properties/profiles/resources (`pomStateFor`, `pomRebuildWorthy`) — so formatting-only edits are ignored. A rebuild-worthy change runs the build (`mvn clean package -DskipTests`, overridable via `MULE_REACTOR_BUILD_COMMAND`, jar selected by artifactId/newest) and deploys the jar; the baseline only advances on success. `--no-watch-pom` prints a stale-app warning instead.
